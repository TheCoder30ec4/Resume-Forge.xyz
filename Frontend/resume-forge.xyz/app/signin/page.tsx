...
import { register, login, googleLoginUrl, ApiError } from "@/lib/api";
...
function SignInInner() {
  ...
  // Redirect already-logged-in users away from signin
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (err) toast.error(`Google login failed: ${err}`);
  }, [params]);
  ...
  function validate() {
    ...
    if (isNewUser && !confirmPassword) e.confirmPassword = "Please confirm your password.";
    else if (isNewUser && password !== confirmPassword) e.confirmPassword = "Passwords do not match.";
    return e;
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const tokens = isNewUser
        ? await register(email, password)
        : await login(email, password);
      const dest = await setTokens(tokens.access_token, tokens.refresh_token);
      router.push(isNewUser ? "/profile-setup" : `/${dest}`);
    } catch (err) {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex flex-col justify-center px-8 md:px-16 py-16 max-w-lg">
        ...
        <p className="text-sm text-muted-foreground mb-8">
          {isNewUser ? "Start free. No credit card required." : "Enter your credentials to continue."}
        </p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {isNewUser && (
            <div className="flex flex-col gap-2">
              <Label htmlFor="confirm-password" className="text-xs uppercase tracking-widest">Confirm Password</Label>
              <div className="relative">
                <Input
                  id="confirm-password"
          ...
          <Button
            type="submit"
            disabled={loading}
            className="h-11 font-bold text-sm transition-colors duration-150 cursor-pointer border-0 mt-1 disabled:opacity-50"
      ...
      >
        ...
        <div className="relative z-10">
          <p className="text-4xl font-extrabold tracking-tighter leading-tight mb-6" style={{ color: "oklch(0.96 0.005 80)" }}>
            Your résumé,<br />
